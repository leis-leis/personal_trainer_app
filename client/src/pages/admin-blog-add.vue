...
const addExercise = async () => {
  ...
  const res = await axios.post("http://127.0.0.1:4000/api/posts/postadd",
        formData,
        {
...
<template>
  <v-container style="display: flex; flex-direction: row; justify-content: space-around;">
    <v-btn @click="posts" variant="tonal">
      Posty
    </v-btn>
    <h1>Dodaj post</h1>
  </v-container>
  ...
  <v-container style="width: 50%;">
    <v-container>
        ...
        <v-container>
            <v-text-field v-model="title" label="Tytuł postu"></v-text-field>
          
            <v-text-field v-model="desc" label="Opis"></v-text-field>
